...
SCAN_INTERVAL_KZ  = int(os.environ.get("SCAN_INTERVAL_KZ",  "900"))
SCAN_INTERVAL_OUT = int(os.environ.get("SCAN_INTERVAL_OUT", "1800"))
...
def minutes_to_next_kz():
    ...
    m   = now.hour * 60 + now.minute
    best_d, best_n = 9999, ""
    for sh, sm, eh, em, name in KILL_ZONES = sh * 60 + sm - m
        if d < 0:
            d += 1440
        if d < best_d = d, name
...
def get_state(sym):
    if sym not in pair_states = {"state": "WAITING_FOR_SWEEP"}
...
def get_all_tickers():
    try = requests.get(f"{MEXC_BASE}/ticker", timeout=15)
        r.raise_for_status()
        d = r.json()
...
def get_price(symbol):
    try = requests.get(f"{MEXC_BASE}/ticker?symbol={symbol}", timeout=8)
...
def location_4h(h4):
    ...
    if sh is None or sl is None or sh == sl = h4[-20:]
...
def detect_displacement(m15, bias, after_idx):
    ...
    for dc in disp_candles = abs(dc["close"] - dc["open"])
        rng  = dc["high"] - dc["low"]
        if rng == 0:
            continue
        br     = body / rng
        ...
        if body >= avg * 1.8 and br >= 0.5 and is_dir = False
            try = m15.index(dc)
                if 0 < idx < len(m15) - 2 = m15[idx - 1]
                    nc = m15[idx + 1]
...
def is_expansion_candle(candle, avg_body, bias, prev_candle):
    ...
    imb = False
    if prev_candle == "bullish" and candle["low"]  > prev_candle["high"]:
            imb = True
...
def detect_bos(m15, bias, sweep_idx, avg_body, location):
    ...
    if location == "equilibrium":
        return {"broken": False, "reason": "BOS at equilibrium -- low probability"}
...
def find_liquidity_targets(m15, h4, daily, bias):
    ...
    if bias == "bullish":
        ...
        targets.sort(key=lambda x =True)
    return targets
...
def build_status_digest(scored_pairs):
    ...
    kz   = f"In kill zone: {zone}" if zone else "Outside kill zones (30-min scans active)"
    ...
    if active:
        ...
        for sym, st in active = st.get("state", "?")
    ...
    d, n = minutes_to_next_kz()
    lines.append(f"Next kill zone: {n} in {d//60}h {d%60}m")
    return "\n".join(lines)
...
def run_state_machine(sym, m15, h4, daily, bd, loc_data, structure, pair_score):
    ...
    if st["state"] == "SWEEP_CONFIRMED":
        if state_age(st, "sweep_time") > MAX_SWEEP_AGE = MAX_SWEEP_AGE // 60
    ...
    if st["state"] == "WAITING_FOR_RETRACEMENT":
        if state_age(st, "bos_time") > MAX_BOS_AGE = st.get("ote", {})
            mins = MAX_BOS_AGE // 60
            record_rejection(
        ...
        if not in_zone:
            record_rejection(
                sym, "Step 7 (OTE)",
                f"Price {price:.4f} not yet in OTE "
                f"{ote['ote_low']:.4f}-{ote['ote_high']:.4f}"
            )
            log.info(f"{sym}: [WAITING_FOR_RETRACEMENT] price {price:.4f} not in OTE")
            return
        ...
        if reacted:
            send_telegram(
                f"<b>ENTRY CONFIRMED -- {sym.replace('_', '/')} {bias.upper()}</b>\n\n"
            ...
            log.info(f"{sym}: ENTRY CONFIRMED at {price:.4f}")
        ...
        if st["checks"] >= 3:
            ...
            send_telegram(
                ...
                f"<i>Rule = skip. Do not chase. State reset.</i>"
            )
            return

        log.info(f"{sym}: [WAITING_FOR_RETRACEMENT] check {st['checks']}/3 at {price:.4f}")
...
def scan_pair(sym, pair_score):
    ...
    if (bias == "bullish" and loc == "premium") or \
       (bias == "bearish" and loc == "discount"):
        st = get_state(sym)
        if st["state"] != "WAITING_FOR_SWEEP":
            reset_state(sym, f"Location misaligned ({loc})")
        record_rejection(sym, "Step 2 (4H Location)",
                         f"Bias is {bias} but price in {loc}")
        log.info(f"{sym}: misaligned ({loc})")
        return
    if loc == "unknown":
    ...
    if st["state"] == "WAITING_FOR_SWEEP":
        ...
        if not structure["clean"]:
            ...
            return
    else = {"clean": True, "reason": "In active setup -- filter bypassed"}
...
def scan_all():
    ...
    for sym in all_pairs:
        try = scored_map.get(sym, {"score": 0, "change_pct": 0, "reason": ""})
            scan_pair(sym, ps)
            time.sleep(1.2)
        except Exception as e:
            log.error(f"{sym}: {e}")
...
if __name__ == "__main__":
    log.info("BEE-M Alert Bot v8 starting...")

    startup_lines = [
        "<b>BEE-M Alert Bot v8 -- Full State Machine</b>",
        "",
        "Core truth: Liquidity taken -> Market shifts -> Enter on retracement",
        "",
        "<b>State machine per pair (remembered across cycles):</b>",
        "  WAITING_FOR_SWEEP",
        "  -> SWEEP_CONFIRMED",
        "  -> WAITING_FOR_RETRACEMENT",
        "  -> TRADE_TAKEN",
        ...
        "<b>24/7 scanning:</b>",
        "  Kill zones: full scan every 15 min",
        "  Outside kill zones: scan every 30 min (always running)",
        "  Status digest every 2 hours",
        "  Rejection reasons sent for every scanned pair",
        "",
        "<b>Kill zones (WAT):</b>",
        "  London:          03:00-06:00",
        "  New York:        08:00-11:00",
        "  MEXC 8AM spike:  08:00-08:30",
        "  MEXC afternoon:  14:30-15:30",
        "  MEXC daily reset: 17:00-18:00",
        "",
        "Any rule not met = NO TRADE.",
    ...
    while True:
        try = current_kill_zone()
            ...
            if zone and zone != last_zone:
                ...
                send_telegram(
                    f"<b>Kill zone open -- {zone}</b>\n"
                    f"Full scan starting now. {t}"
                )
                last_zone = zone

            if not zone and last_zone = minutes_to_next_kz()
                send_telegram(
                    f"<b>{last_zone} closed</b>\n"
                    f"Scanning continues every 30 min.\n"
                    f"Next kill zone: {n} in {d//60}h {d%60}m"
                )
                last_zone = ""

            last_scored = scan_all()

            # Status digest every STATUS_INTERVAL seconds
            global last_status_time
            if now - last_status_time >= STATUS_INTERVAL = now
                digest = build_status_digest(last_scored)
                send_telegram(digest)

            sleep_secs = SCAN_INTERVAL_KZ if zone else SCAN_INTERVAL_OUT
            log.info(f"Sleeping {sleep_secs}s ({'kill zone' if zone else 'outside KZ'})")
