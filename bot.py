...
def find_4h_swing_points(h4: list) -> tuple:
    """
    Find the most recent SIGNIFICANT 4H swing high and swing low
    using proper pivot logic (candle must be higher/lower than
    2 neighbours on each side).

    This gives us the actual dealing range institutions are working
    within — not just an arbitrary 20-candle window.

    Returns (swing_high_price, swing_low_price, swing_high_idx, swing_low_idx)
    or (None, None, None, None) if not enough structure.
    """
    if len(h4) < 10:
    ...
    # Walk backwards to find the most recent swing high AND swing low
    for i in range(len(h4) - 3, 1, -1):
        c = h4[i]

        # Swing high — higher than 2 candles on each side
        if (sh_price is None and
            i >= 2 and i <= len(h4) - 3 and
...
def location_4h(h4: list) -> dict:
    """
    Determine 4H premium/discount using a Fibonacci tool applied
    to the actual 4H swing structure — exactly as you would on
    TradingView.

    Steps:
      1. Find the most recent 4H swing high and swing low
      2. Apply Fibonacci retracement between them
      3. 50% level = equilibrium
      4. Above 50% = premium  (sell-side, avoid longs here)
      5. Below 50% = discount  (buy-side, favour longs here)

    Fib levels returned so the alert can show them:
      0%    = swing low  (or swing high for bearish leg)
      23.6% = shallow retracement
      38.2% = first meaningful level
      50%   = equilibrium
      61.8% = OTE start
      70.5% = OTE ideal
      79.2% = OTE end
      100%  = swing high (or swing low for bearish leg)

    Returns dict with location string and fib levels.
    """
    sh, sl, sh_idx, sl_idx = find_4h_swing_points(h4)

    # Fallback to simple midpoint if no clear swings found
    if sh is None or sl is None or sh == sl:
        r   = h4[-20:]
        sh  = max(c["high"] for c in r)
        sl  = min(c["low"]  for c in r)
        log.debug("location_4h: no clear pivots, using 20-candle range fallback")
    ...
    if rng == 0:
        return {"location": "unknown", "eq": price, "sh": sh, "sl": sl,
                "fib_236": price, "fib_382": price, "fib_618": price,
                "fib_705": price, "fib_792": price}

    # Determine which came first — is this an upswing or downswing leg?
    # Upswing leg:   swing low happened before swing high (bullish move)
    # Downswing leg: swing high happened before swing low (bearish move)
    if sh_idx is not None and sl_idx is not None:
        bullish_leg = sl_idx < sh_idx   # low formed first, then high
    else:
        bullish_leg = True  # default assumption

    eq      = sl + 0.5   * rng   # 50% — equilibrium
    fib_236 = sl + 0.236 * rng
    fib_382 = sl + 0.382 * rng
    fib_618 = sl + 0.618 * rng   # OTE start
    fib_705 = sl + 0.705 * rng   # OTE ideal
    fib_792 = sl + 0.792 * rng   # OTE end
...
# ═══════════════════════════════════════════════════════════════
# SHALLOW RETRACEMENT — FVG CONTINUATION ENTRY
#
# When the market is in strong expansion, price often doesn't
# pull back deep enough to reach the OTE zone (0.618–0.792).
# It may only retrace 0.2–0.4 before continuing.
#
# In this case, instead of waiting for OTE and getting left behind,
# we look for the nearest FVG (imbalance) left behind during the
# expansion move — even if shallow — and use that as the entry.
#
# Triggers when:
#   - OTE zone is calculated but price has not reached it yet
#   - Last 3 candles show no pullback (still expanding)
#   - An unmitigated FVG exists between current price and OTE
#
# Entry = midpoint of that FVG
# This is sometimes called the "breaker entry" or "FVG continuation"
# ═══════════════════════════════════════════════════════════════

def is_strong_expansion(m15: list, bias: str, after_idx: int) -> bool:
    """
    Detect if price is in continued strong expansion after BOS
    (i.e. not pulling back — shallow retracement scenario).

    Criteria:
    - Last 3 post-BOS candles are all in the right direction
    - Each consecutive candle closes further than the previous
    - No candle has retraced more than 30% of its own body
    """
    post = m15[after_idx + 1:]
    if len(post) < 3:
        return False
    ...
    for c in last3:
        ...
        if rng == 0:
            return False
        # Candle body must be dominant (>50% of range) — no reversal wicks
        if body / rng < 0.5:
            return False
    ...
    # Consecutive closes moving further in direction
...
def find_continuation_fvg(m15: list, bias: str,
                            after_idx: int, ote: dict) -> dict:
    """
    Find the nearest unmitigated FVG between current price and OTE zone.
    This is the shallow retracement entry target.

    For longs:  FVG must be BELOW current price (price will pull back to it)
    For shorts: FVG must be ABOVE current price (price will rally to it)

    The FVG must be:
    - Left behind AFTER the BOS (part of the expansion move)
    - Not yet mitigated (price hasn't returned to fill it)
    - Between current price and the OTE zone
    """
    post     = m15[after_idx:]
    if len(post) < 3:
        return {"found": False}
    ...
    if bias == "bullish":
        ote_low = ote.get("ote_low", 0)
        # FVG below current price and above OTE low (shallow zone)
        candidates = [
            f for f in fvgs
            if f["direction"] == "bullish"
            and f["fvg_low"] < price        # below current price
            and f["fvg_high"] > ote_low     # above OTE low
        ]
        if candidates:
            # Use the highest one (closest to current price = shallowest)
            best = max(candidates, key=lambda x: x["fvg_low"])
            mid  = (best["fvg_high"] + best["fvg_low"]) / 2
            return {
                "found":    True,
                "fvg_high": best["fvg_high"],
                "fvg_low":  best["fvg_low"],
                "entry":    mid,
                "type":     "FVG continuation (shallow retracement)",
            }

    if bias == "bearish":
        ote_high = ote.get("ote_high", float("inf"))
        candidates = [
            f for f in fvgs
            if f["direction"] == "bearish"
            and f["fvg_high"] > price       # above current price
            and f["fvg_low"] < ote_high     # below OTE high
        ]
        if candidates:
            best = min(candidates, key=lambda x: x["fvg_high"])
            mid  = (best["fvg_high"] + best["fvg_low"]) / 2
            return {
                "found":    True,
                "fvg_high": best["fvg_high"],
...
def build_alert(sym: str, bd: dict, loc: str, liq: dict,
                ...
                pair_score: dict, loc_data: dict = None,
                cont_entry: dict = None) -> str:
    ...
    # Continuation / shallow retracement entry line
    if cont_entry and cont_entry.get("found"):
        cont_line = (
            f"\n⚡ <b>Shallow retracement detected</b> — price in strong expansion\n"
            f"Continuation FVG entry: <b>{cont_entry['fvg_low']:.4f}–{cont_entry['fvg_high']:.4f}</b>\n"
            f"Entry midpoint: <b>{cont_entry['entry']:.4f}</b> (use instead of OTE if price won't retrace)"
        )
    else:
        cont_line = ""

    # 4H Fib levels line
    if loc_data and loc_data.get("eq"):
        fib_line = (
            f"4H Fib levels — EQ: <b>{loc_data['eq']:.4f}</b>  "
            f"| 61.8%: <b>{loc_data.get('fib_618',0):.4f}</b>  "
            f"| 79.2%: <b>{loc_data.get('fib_792',0):.4f}</b>  "
            f"| Range: <b>{loc_data.get('sl',0):.4f}</b> → <b>{loc_data.get('sh',0):.4f}</b>"
        )
    else:
        fib_line = ""
...
<b>━━ STEP 2 — 4H LOCATION (Fib on swing structure) ━━</b>
...
def scan_pair(sym: str, pair_score: dict) -> None:
    ...
    # Step 5 — OTE zone + FVG inside OTE (primary entry)
    ...
    # Step 5b — Shallow retracement / FVG continuation check
    # If price is in strong expansion and won't reach OTE, find nearest FVG
    cont_entry = {"found": False}
    if is_strong_expansion(m15, bias, si):
        cont_entry = find_continuation_fvg(m15, bias, si, ote)
        if cont_entry["found"]:
            log.info(f"{sym}: strong expansion detected — continuation FVG entry at {cont_entry['entry']:.4f}")

    # Step 6 — Risk levels
    # Use continuation FVG entry if found, otherwise use OTE ideal
    if cont_entry["found"]:
        # Build a modified ote dict using continuation entry as ideal
        ote_for_risk = dict(ote)
        ote_for_risk["ideal"] = cont_entry["entry"]
        risk = calc_risk(bias, swp, bos, ote_for_risk)
    else:
    ...
    msg = build_alert(sym, bd, loc, liq, swp, tier, bos, fvg_ote, ote, risk, pair_score, loc_data, cont_entry)
    send_telegram(msg)
...
#
# Full scans only run during high-probability windows:
#
#   London open       02:00 – 05:00 UTC  institutional flow, sweeps PDH/PDL
#   New York open     07:00 – 10:00 UTC  highest volume, most displacement
#   MEXC 8AM spike    08:00 – 08:30 UTC  observed high-activity window
#   MEXC afternoon    14:30 – 15:30 UTC  observed high-activity window
#   MEXC daily reset  16:30 – 17:30 UTC  MEXC resets daily stats at 5PM WAT
#
# Outside these windows: OTE monitor still runs (so you never miss
# a CONFIRMED or SKIP alert on a setup that already fired), but no
# new full scans are triggered.
#
# All times are UTC. WAT (West Africa Time) = UTC+1.
# So 5PM WAT = 16:00 UTC. Buffer added either side.
# ═══════════════════════════════════════════════════════════════

# Each window: (start_hour_utc, start_min, end_hour_utc, end_min)
KILL_ZONES = [
...
def current_kill_zone() -> str:
    """
    Returns the name of the active kill zone, or empty string if
    we are outside all trading windows.
    """
...
def minutes_to_next_kill_zone() -> tuple:
    """
    Returns (minutes_away, zone_name) for the next upcoming kill zone.
    """
    now     = datetime.now(timezone.utc)
    h, m    = now.hour, now.minute
    minutes = h * 60 + m
...
def notify_kill_zone_open(zone_name: str) -> None:
    send_telegram(
        f"🔔 <b>Kill zone open — {zone_name}</b>\n"
        f"Starting full scan now. Watching all MEXC futures pairs.\n"
        f"<i>{datetime.now(timezone.utc).strftime('%H:%M UTC')}</i>"
    )


def notify_kill_zone_closed(zone_name: str) -> None:
    send_telegram(
        f"🔕 <b>{zone_name} closed</b>\n"
        f"Full scans paused. OTE monitor stays active.\n"
        f"<i>Next kill zone coming up — bot will notify you when it opens.</i>"
    )


if __name__ == "__main__":
    log.info("BEE-M Alert Bot v5 starting...")
    send_telegram(
        "🤖 <b>BEE-M Alert Bot v5 — Session-Aware</b>\n\n"
        "<b>Kill zones (WAT):</b>\n"
        "🕒 London open:      03:00 – 06:00 WAT\n"
        "🕖 New York open:    08:00 – 11:00 WAT\n"
        "🕗 MEXC 8AM spike:   08:00 – 08:30 WAT\n"
        "🕑 MEXC afternoon:   14:30 – 15:30 WAT\n"
        "🕔 MEXC daily reset: 17:00 – 18:00 WAT\n\n"
        ...
        "<b>Meaningful BOS — 5 criteria:</b>\n"
        "✔ Relevant swing structure only\n"
        "✔ Candle closes beyond level\n"
        "✔ Expansion BOS only (body dominant, leaves FVG)\n"
        "✔ Only after liquidity sweep\n"
        "✔ Location aligned (discount for longs, premium for shorts)\n\n"
        "<b>All BEE-M rules active:</b>\n"
        "✔ 8 daily bias scenarios\n"
        "✔ 4H location, liquidity path, sweep tier\n"
        "✔ FVG inside OTE, 3-candle skip rule\n"
        "✔ SL at sweep extreme or BOS origin\n"
        "✔ TP1 (50%) / TP2 (30%) / Runner (20%)\n\n"
        "<i>Full scans run during kill zones only.\n"
        "OTE monitoring runs 24/7.\n"
        "Any rule not met = NO TRADE.</i>"
    )

    last_zone       = ""   # track zone transitions
    last_notified_next = "" # avoid repeating "next zone" messages

    while True:
        try:
            zone = current_kill_zone()

            # ── Kill zone just opened ─────────────────────────
            if zone and zone != last_zone:
                log.info(f"Kill zone opened: {zone}")
                notify_kill_zone_open(zone)
                last_zone = zone

            # ── Kill zone just closed ─────────────────────────
            if not zone and last_zone:
                log.info(f"Kill zone closed: {last_zone}")
                notify_kill_zone_closed(last_zone)
                last_zone = ""

                # Tell user when the next one is
                diff_min, next_name = minutes_to_next_kill_zone()
                hrs  = diff_min // 60
                mins = diff_min % 60
                send_telegram(
                    f"⏱ <b>Next kill zone: {next_name}</b>\n"
                    f"Opens in {hrs}h {mins}m\n"
                    f"<i>OTE monitor is still running in the background.</i>"
                )

            # ── Inside a kill zone → full scan ────────────────
            if zone:
                log.info(f"[{zone}] Running full scan...")
                scan_all()
                sleep_secs = SCAN_INTERVAL
                log.info(f"Scan done. Sleeping {sleep_secs}s...")
                time.sleep(sleep_secs)

            # ── Outside kill zones → OTE monitor only ─────────
            else:
                if ote_watch_queue:
                    log.info("Outside kill zone — running OTE monitor only...")
                    monitor_ote_queue()
                else:
                    diff_min, next_name = minutes_to_next_kill_zone()
                    log.info(
                        f"Outside kill zone. Next: {next_name} "
                        f"in {diff_min // 60}h {diff_min % 60}m. Sleeping 60s..."
                    )
                time.sleep(60)
