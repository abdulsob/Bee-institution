...
def _model4_momentum_phase() -> str:
    """
    Return the current Model 4 momentum phase based on UTC hour.
    3-phase system (PDF1 Fix 5):
      EARLY_BREAKOUT  — fresh momentum, early entry allowed
      MID_EXPANSION   — confirmed momentum, main trade zone
      LATE_EXHAUSTION — momentum fading, exhaustion checks tighten
      LOW             — outside active momentum hours
    """
...
def update_model4_state(sym: str, m15: list, bias: str) -> None:
    ...
    # ── Model 4 activation (strict — per PDF, upgraded to 3-phase) ──────────
    threshold = 0.6 if st["is_priority_pair"] else 0.7

    # EARLY_BREAKOUT: allow early entry when strong volume (imbalance scanner hot)
    scan = scanner_state.get(sym, {})
    early_ok = (
        st["momentum_phase"] == "EARLY_BREAKOUT"
        and scan.get("is_hot")
        and impulse_strength > threshold
    )
    mid_ok = (
        st["momentum_phase"] == "MID_EXPANSION"
        and st["no_dump"]
        and impulse_strength > threshold
    )
    late_ok = (
        st["momentum_phase"] == "LATE_EXHAUSTION"
        and st["no_dump"]
        and impulse_strength > threshold * 1.1   # tighter in late phase
    )
    if early_ok or mid_ok or late_ok:
    ...
    # ── Exhaustion tightens in LATE_EXHAUSTION phase ──────────────────────
    st["exhaustion"] = False
    exhaustion_is_top_gainer = st["is_top_gainer"]
    exhaustion_late = st["momentum_phase"] == "LATE_EXHAUSTION"
    if exhaustion_is_top_gainer or exhaustion_late:
        if retrace_depth > (0.5 if exhaustion_late else 0.6):
            st["exhaustion"] = True
        if impulse_strength < (0.55 if exhaustion_late else 0.5):
            st["exhaustion"] = True
        consec = _model4_consecutive_large_candles(m15, bias)
        if consec >= (2 if exhaustion_late else 3):
            st["exhaustion"] = True

    # ── Liquidity void TP bonus (PDF2 §9) ─────────────────────────────────
    # If the imbalance scanner found a liquidity void, TP multiplier gets +0.2
    st["liquidity_void_bonus"] = scan.get("liquidity_void", False)
...
def model4_get_tp_multiplier(st: dict) -> float:
    ...
    PDF2 §9: +0.2 bonus if liquidity void detected by imbalance scanner.
    """
    if not st.get("momentum_model"):
        return 1.0
    base = 1.5 if st.get("is_top_loser") else 1.8
    if st.get("liquidity_void_bonus"):
        base += 0.2
    return base
...
# ═══════════════════════════════════════════════════════════════════════════
# INTELLIGENCE LAYER — Imbalance Scanner + Hot Pair Engine (PDF2 unified arch)
#
# This is the intelligence layer ABOVE the existing state machine.
# It does NOT replace any core logic — it annotates pairs before the
# state machine runs, giving the engine richer context to act on.
# ═══════════════════════════════════════════════════════════════════════════

# Per-pair imbalance scan results. Updated each cycle.
# scanner_state[sym] = {fvg, displacement, momentum_shift, score, is_hot, direction, ...}
scanner_state: dict = {}


def scan_imbalances(sym: str, m15: list) -> dict:
    """
    Detect FVG, displacement, momentum shift, and liquidity void on 15M.
    Returns a result dict — does NOT modify state itself.
    Called every cycle before run_state_machine.
    """
    result = {
        "fvg":             False,
    ...
    # ── Displacement (last candle > 2x avg range) ────────────────────────
    avg_range = sum(c["high"] - c["low"] for c in m15[-20:]) / 20
    last = m15[-1]
    if avg_range > 0 and (last["high"] - last["low"]) > avg_range * 2:
        result["displacement"] = True

    # ── Momentum shift (3 consecutive directional closes) ────────────────
    last3 = m15[-3:]
    bull  = sum(1 for c in last3 if c["close"] > c["open"])
    bear  = sum(1 for c in last3 if c["close"] < c["open"])
    if bull == 3:
        result["momentum_shift"] = "bullish"
    elif bear == 3:
        result["momentum_shift"] = "bearish"

    # ── Liquidity void (displacement + FVG together) ─────────────────────
    if result["displacement"] and result["fvg"]:
        result["liquidity_void"] = True

    # ── Hot score ─────────────────────────────────────────────────────────
    if result["fvg"]:            result["score"] += 2
    if result["displacement"]:   result["score"] += 2
    if result["momentum_shift"]: result["score"] += 2
    result["is_hot"] = result["score"] >= 4

    return result


def _apply_imbalance_bonus(pair_score: dict, sym: str) -> dict:
    """
    Merge the imbalance scanner score into the pair's score dict.
    Used inside scan_pair to enrich the scoring before state machine runs.
    """
    scan = scanner_state.get(sym, {})
    if not scan:
        return pair_score
    enriched = dict(pair_score)
    enriched["imb_score"]      = scan.get("score", 0)
    enriched["is_hot"]         = scan.get("is_hot", False)
    enriched["scan_direction"] = scan.get("direction")
    enriched["liquidity_void"] = scan.get("liquidity_void", False)
    enriched["score"]          = pair_score.get("score", 0) + scan.get("score", 0)
    return enriched



def _volatility_ok(m15: list):
    """
...
# ═══════════════════════════════════════════════════════════════════════════
# DUAL BIAS ENGINE (PDF1 Fix 1)
#
# Primary bias = HTF (daily) — the structural direction
# Execution bias = LTF reality — what price is actually doing NOW
#
# Key rule: if strong_trend, allow continuation even if counter-HTF bias.
# This prevents the bot from fighting strong momentum.
# ═══════════════════════════════════════════════════════════════════════════

def resolve_execution_bias(primary_bias: str, mc: dict, flags: dict, scan: dict) -> str:
    """
    Resolve the execution bias — what the bot should actually trade.
    In most cases this matches primary_bias (HTF).
    When strong trend momentum is confirmed LTF, allow continuation
    even if it goes counter to the HTF bias.

    Returns: "bullish", "bearish", or primary_bias (default passthrough).
    """
    condition    = mc.get("condition", "ranging")
    trend_dir    = mc.get("trend_dir", "none")
    strong_trend = flags.get("strong_trend", False)
    scan_dir     = scan.get("direction")

    # Strong trend override: price is in a confirmed trending state AND
    # both the market condition and imbalance scanner agree on direction.
    if strong_trend and condition == "trending":
        if trend_dir == "up" and scan_dir == "bullish":
            return "bullish"
        if trend_dir == "down" and scan_dir == "bearish":
            return "bearish"

    # Default: honour the HTF (primary) bias
    return primary_bias


# ═══════════════════════════════════════════════════════════════════════════
# ADAPTIVE BOS THRESHOLD (PDF1 Fix 3)
#
# strong_trend = 0.45  (lower barrier to catch trend continuation trades)
# hot pair     = 0.55  (imbalance scanner confirmed momentum)
# default      = 0.60  (standard Model 4 value)
# no context   = 0.70  (no London sweep / poor context)
# ═══════════════════════════════════════════════════════════════════════════

def adaptive_bos_threshold(st: dict, flags: dict, scan: dict) -> float:
    """Return the BOS body-ratio threshold appropriate for current conditions."""
    if flags.get("strong_trend"):
        return 0.45
    if scan.get("is_hot"):
        return 0.55
    if st.get("momentum_model"):
...
# ═══════════════════════════════════════════════════════════════════════════
# IMPERFECT SETUP SCORING (PDF1 Fix 6)
#
# Real traders adapt — not every trade needs perfect alignment.
# Score the available evidence. If ≥ 4 → allow trade.
# This increases frequency while keeping quality gates.
# ═══════════════════════════════════════════════════════════════════════════

def imperfect_setup_score(sweep: dict, bos: dict, retrace_ok: bool,
                          in_session: bool, scan: dict) -> dict:
    """
    Score a setup based on available components. Returns dict with score and label.
    Threshold: score >= 4 allows trade entry.
    """
    score = 0
    parts = []
    ...
    if sweep and sweep.get("swept"):
        score += 2
        parts.append("Sweep ✔ (+2)")

    if bos and bos.get("broken"):
        score += 2
        parts.append("BOS ✔ (+2)")

    if retrace_ok:
        score += 1
        parts.append("Retrace ✔ (+1)")

    if in_session:
        score += 1
        parts.append("Session ✔ (+1)")

    if scan.get("is_hot"):
        score += 1
        parts.append("Hot pair ✔ (+1)")
    ...
    return {
        ...
        "allowed": allowed,
        "label":   " | ".join(parts),
        "grade":   "A+" if score >= 6 else "A" if score >= 5 else "B" if score >= 4 else "C",
    }
...
def get_state(sym):
    if sym not in pair_states:
        pair_states[sym] = {
            ...
            "exhaustion":         False,
            "tp_target":          None,
            "liquidity_void_bonus": False,
            # ── Dual bias (PDF1 Fix 1) ─────────────────────────────────────
            "execution_bias":     None,   # LTF reality — may differ from HTF bias
        }
...
def run_state_machine(sym, m15, h4, daily, bd, loc_data, structure, pair_score, market_condition=None, live_price=None):
    ...
    if st["state"] == "SWEEP_CONFIRMED":
        ...
        # ── BOS STRENGTH FILTER — Adaptive (PDF1 Fix 3 + Model 4 §11) ────────
        # strong_trend → 0.45 | hot pair → 0.55 | momentum → 0.60 | default → 0.70
        bos_body_ratio   = bos.get("body_ratio", 0)
        flags_bos        = _ensure_session_flags_fresh(sym)
        scan_bos         = scanner_state.get(sym, {})
        m4_bos_threshold = adaptive_bos_threshold(st, flags_bos, scan_bos)
        ...
        # ── No London sweep — require A+ grade or strong context ─────────────
        if not flags_bos["london_sweep"]:
            setup_grade_check = score_setup(sweep, disp, bos, {}, structure).get("rating", "B")
            if setup_grade_check == "B" and not flags_bos["strong_trend"] and not scan_bos.get("is_hot"):
                record_rejection(sym, "Step 6 (BOS — no London sweep)",
                                 "No London sweep — B-grade setup blocked (need A+, strong trend, or hot pair)")
                log.info(f"{sym}: NO LONDON SWEEP — B-grade blocked, need A+/strong trend/hot pair")
                return
    ...
    if st["state"] == "WAITING_FOR_RETRACEMENT":
        ...
        ote     = st["ote"]
        risk    = st["risk"]
        bos     = st.get("bos", {})   # read BOS from state — it was confirmed in SWEEP_CONFIRMED

        # ── MODEL 4 §11: min_retrace widens the entry zone for priority pairs ──
        # Standard OTE is 0.5–0.786 retrace. When Model 4 + priority pair active,
        # allow entry from 0.25 retrace (earlier entry on momentum).
        m4_min_retrace = model4_get_min_retrace(st)
        if m4_min_retrace < 0.5 and st.get("momentum_model"):
            # Widen ote_low (bullish) or ote_high (bearish) to the shallower retrace level
            sweep  = st.get("sweep", {})
            bos_d  = st.get("bos", {})
            bos_level   = bos_d.get("bos_level", 0)
            sweep_level = sweep.get("sweep_low", sweep.get("sweep_high", 0))
            if bos_level and sweep_level:
                rng = abs(bos_level - sweep_level)
                if bias == "bullish":
                    extended_low = bos_level - rng * m4_min_retrace
                    ote = dict(ote)
                    ote["ote_low"] = min(ote["ote_low"], extended_low)
                else:
                    extended_high = bos_level + rng * m4_min_retrace
                    ote = dict(ote)
                    ote["ote_high"] = max(ote["ote_high"], extended_high)
            log.debug(f"{sym}: Model4 min_retrace={m4_min_retrace} — OTE zone widened")

        in_zone = ote["ote_low"] <= price <= ote["ote_high"]

        # ── IMPERFECT SETUP SCORING (PDF1 Fix 6) ──────────────────────────
        # If not in OTE zone, check if imperfect setup score allows entry anyway.
        # sweep+bos+retrace+session+hot → score ≥ 4 → allow trade.
        retrace_ok     = in_zone  # within OTE counts as retrace confirmed
        in_session_now = in_execution_window()[0]
        scan_entry     = scanner_state.get(sym, {})
        imp_score      = imperfect_setup_score(
            st.get("sweep", {}), bos, retrace_ok, in_session_now, scan_entry
        )
        # Allow entry even outside strict OTE if imperfect score meets threshold
        if not in_zone and imp_score["allowed"] and scan_entry.get("is_hot"):
            log.info(f"{sym}: IMPERFECT ENTRY — score {imp_score['score']} | {imp_score['label']}")
            in_zone = True  # hot pair with sufficient score unlocks entry
        ...
        if not in_zone:
            ...
            if mom["valid"]:
                ...
                scan_b    = scanner_state.get(sym, {})
                hot_tag_b = "\n🔥 <b>HOT PAIR</b> — Imbalance scanner confirmed momentum" if scan_b.get("is_hot") else ""
                exec_bias = st.get("execution_bias", bias)
                prio_tag_b = f"  [Priority: {st.get('priority', 'NORMAL')}]"

                risk_display = dict(risk_b)
                if m4_tp_mult != 1.0 and st.get("momentum_model"):
                    entry = risk_display.get("entry", price)
                    if bias == "bullish":
                        risk_display["tp1"]    = entry + (risk_b["tp1"]    - entry) * m4_tp_mult
                        risk_display["tp2"]    = entry + (risk_b["tp2"]    - entry) * m4_tp_mult
                        risk_display["runner"] = entry + (risk_b["runner"] - entry) * m4_tp_mult
                    else:
                        risk_display["tp1"]    = entry - (entry - risk_b["tp1"])    * m4_tp_mult
                        risk_display["tp2"]    = entry - (entry - risk_b["tp2"])    * m4_tp_mult
                        risk_display["runner"] = entry - (entry - risk_b["runner"]) * m4_tp_mult
                ...
                if st.get("momentum_model"):
                    ...
                    if m4_tp_mult != 1.0:
                        lv_note = " + liquidity void" if st.get("liquidity_void_bonus") else ""
                        m4_tp_line += f"  [TP x{m4_tp_mult}{lv_note}]"

                send_telegram(
                    f"<b>🔥 ENTRY — Model B — {sym.replace('_', '/')} {exec_bias.upper()}</b>"
                    f"{prio_tag_b}\n"
                    f"<i>{mom['reason']}</i>\n"
                    f"Window: {win_tag}{hot_tag_b}\n\n"
                    f"<b>Narrative:</b>  {narr.get('expect', '')}\n"
                    f"Invalidation: {narr.get('inval', '')}\n\n"
                    f"Price:        <b>{price:.4f}</b>  (momentum — no OTE retrace)\n"
                    f"Entry:        <b>{risk_display['entry']:.4f}</b>\n"
                    f"SL:           <b>{risk_display['sl']:.4f}</b>\n"
                    f"TP1 (50%):    <b>{risk_display['tp1']:.4f}</b>  [{risk_display.get('tp1_label', '--')}]\n"
                    f"TP2 (30%):    <b>{risk_display['tp2']:.4f}</b>  [{risk_display.get('tp2_label', '--')}]\n"
                    f"Runner (20%): <b>{risk_display['runner']:.4f}</b>  [{risk_display.get('runner_label', '--')}]"
                    f"{m4_tp_line}\n\n"
        ...
        if reacted or in_zone:
            in_window, win_name = in_execution_window()
            win_tag = f"🟢 {win_name}" if in_window else "⚠️ Outside execution window — reduced confidence"
            fvg_note = ""
            ...
            # ── Hot pair / priority tag ───────────────────────────────────
            scan_a    = scanner_state.get(sym, {})
            hot_tag   = "\n🔥 <b>HOT PAIR</b> — Imbalance scanner confirmed momentum" if scan_a.get("is_hot") else ""
            prio_tag  = f"  [Priority: {st.get('priority', 'NORMAL')}]"
            exec_bias = st.get("execution_bias", bias)
            bias_tag  = (f"\n⚡ Execution bias: <b>{exec_bias.upper()}</b>"
                         f"  (HTF: {st.get('primary_bias', exec_bias).upper()})"
                         if exec_bias != st.get("primary_bias", exec_bias) else "")
            imp_tag   = (f"\n📊 Imperfect entry: score {imp_score['score']}/7 — {imp_score['label']}"
                         if not (ote["ote_low"] <= price <= ote["ote_high"]) else "")
            ...
            risk_display = dict(risk)
            if m4_tp_mult != 1.0 and st.get("momentum_model"):
                entry = risk_display.get("entry", price)
                if bias == "bullish":
                    risk_display["tp1"]    = entry + (risk["tp1"]    - entry) * m4_tp_mult
                    risk_display["tp2"]    = entry + (risk["tp2"]    - entry) * m4_tp_mult
                    risk_display["runner"] = entry + (risk["runner"] - entry) * m4_tp_mult
                else:
                    risk_display["tp1"]    = entry - (entry - risk["tp1"])    * m4_tp_mult
                    risk_display["tp2"]    = entry - (entry - risk["tp2"])    * m4_tp_mult
                    risk_display["runner"] = entry - (entry - risk["runner"]) * m4_tp_mult
            ...
            send_telegram(
                f"<b>🔥 ENTRY — Model A — {sym.replace('_', '/')} {exec_bias.upper()}</b>"
                f"{prio_tag}\n"
                f"Window: {win_tag}{hot_tag}{bias_tag}{imp_tag}\n\n"
                f"<b>Narrative:</b>  {narr.get('expect', '')}\n"
                f"Invalidation: {narr.get('inval', '')}\n\n"
    ...
    if st["state"] == "TRADE_TAKEN":
        # ── MODEL 4: Exhaustion exit — "Exit Early if exhaustion and in_trade" ──
        # Re-evaluate exhaustion each cycle while in trade (PDF §8)
        update_model4_state(sym, m15, bias)
        if st.get("exhaustion"):
            send_telegram(
                f"<b>🚨 MODEL 4 — EXIT EARLY — {sym.replace('_', '/')} {bias.upper()}</b>\n"
                f"Top gainer showing exhaustion while in trade.\n"
                f"Conditions met: retracement > 0.6 OR impulse strength < 0.5 OR 3+ large candles.\n\n"
                f"<b>Close trade now. Do not hold.</b>"
            )
            log.info(f"{sym}: MODEL 4 EXHAUSTION — exit early alert sent")
...
def scan_pair(sym, pair_score):
    ...
    # ── INTELLIGENCE LAYER: Imbalance scanner (PDF2) ─────────────────────────
    scan = scan_imbalances(sym, m15)
    scanner_state[sym] = {**scan, "timestamp": time.time()}
    pair_score = _apply_imbalance_bonus(pair_score, sym)

    # ── SESSION FLAGS — ensure fresh for today ────────────────────────────────
    flags = _ensure_session_flags_fresh(sym)
    wat_hour, _ = _wat_now()

    # ── VOLATILITY KILL FILTER ────────────────────────────────────────────────
    ...
    # ── PDF1 Fix 5: NY session entry filter is now handled above loc_data ──────

    bd           = daily_bias(daily)
    primary_bias = bd["bias"]

    # ── DUAL BIAS — resolve execution bias (PDF1 Fix 1) ──────────────────────
    # primary_bias = HTF structural direction (daily)
    # execution_bias = what we actually trade (may match or diverge on strong trend)
    mc_early = classify_market_condition(h4, daily, bd)
    bias     = resolve_execution_bias(primary_bias, mc_early, flags, scan)

    # ── INTELLIGENCE LAYER: Direction filter (PDF2 §6D) ──────────────────────
    # If scanner has a confirmed direction AND it conflicts with execution bias,
    # skip this pair — structure and momentum are not aligned.
    # Exception: active setups always pass through (they already confirmed).
    st_direction_check = pair_states.get(sym, {})
    is_active_for_dir  = st_direction_check.get("state") not in (
        None, "WAITING_FOR_SWEEP", "BEHAVIOR_CLASSIFICATION")
    if (not is_active_for_dir
            and scan.get("direction")
            and scan["direction"] != bias):
        record_rejection(sym, "Direction Filter",
                         f"Scanner direction '{scan['direction']}' conflicts with bias '{bias}' — skipping")
        log.info(f"{sym}: DIRECTION FILTER — scanner={scan['direction']} bias={bias}, skipped")
        return
    ...
    if bias == "neutral":
        ...
        record_rejection(sym, "Step 1 (Daily Bias)", f"Neutral bias (score {bd['score']})")
    ...
    if abs(bd["score"]) < 2:
        ...
        record_rejection(sym, "Step 1 (Narrative gate)",
                         f"Bias score {bd['score']:+d} — minimum ±2 required")
    ...
    is_active_setup = st.get("state") not in (None, "WAITING_FOR_SWEEP", "BEHAVIOR_CLASSIFICATION")
    ...
    # ── PDF1 Fix 3: Detect internal sweeps ───────────────────────────────────
    ...
    # ── PDF1 Fix 5: NY session entry filter ───────────────────────────────────
    if in_window_now and is_newyork_session(wat_hour) and not is_active_setup:
        if not flags["london_sweep"]:
            ...
            if not valid_ny_condition:
                ...
                log.info(f"{sym}: NY GATE — blocked")
                return
    ...
    # ── PDF1 Fix 4: Session grading — outside window tags trade, doesn't block ─
    # "Inside window → full size / Outside window → reduced confidence / tag trade"
    # (This is enforced in the alert messages via win_tag — no hard block here)
    outside_window_grade = "" if in_window_now else "⚠️ OUTSIDE WINDOW — reduced confidence"
    ...
    # Store execution bias in state for downstream reference
    st["execution_bias"] = bias
    st["primary_bias"]   = primary_bias

    # ── PDF2 §6A: Tag priority in state ──────────────────────────────────────
    st["priority"] = "HIGH" if scan.get("is_hot") else "NORMAL"
...
if __name__ == "__main__":
    ...
    startup_lines = [
        "<b>BEE-M Alert Bot v19 — Unified Intelligence Layer</b>",
        "",
        "<b>Core flow (UNCHANGED):</b>  Sweep → BOS → Retracement → Entry",
        "<b>Models 1–3:</b>  Reversal / Continuation / Expansion  (structure only)",
        "<b>Model 4:</b>  Momentum modifier — enhances execution, never creates trades",
        "",
        "<b>NEW — PDF1 Upgrades:</b>",
        "  Fix 1: Dual bias — Primary (HTF daily) + Execution (LTF reality)",
        "         Strong trend override: if trending + scanner agrees → trade WITH momentum",
        "  Fix 3: Adaptive BOS threshold — strong_trend=0.45 | hot=0.55 | momentum=0.60 | default=0.70",
        "  Fix 4: Session grading — outside window = tagged, NOT blocked",
        "  Fix 5: Model 4 upgraded to 3 phases: EARLY_BREAKOUT / MID_EXPANSION / LATE_EXHAUSTION",
        "         Early breakout + hot pair unlocks early entry",
        "         Late exhaustion tightens all checks automatically",
        "  Fix 6: Imperfect setup scoring — sweep+bos+retrace+session+hot → score ≥ 4 → trade",
        "",
        "<b>NEW — PDF2 Intelligence Layer:</b>",
        "  Imbalance Scanner: FVG + displacement + momentum shift → hot pair score 0-6",
        "  scanner_state updated every cycle — feeds BOS threshold, entry, TP multiplier",
        "  Direction filter: scanner direction vs execution bias — mismatches skipped",
        "  Liquidity void (FVG + displacement) → TP multiplier +0.2 bonus",
        "  Hot pair tag shown in all entry alerts",
        "  Priority tagging: HIGH (hot) / NORMAL in sweep alert + entry alert",
        "",
        "<b>Existing (preserved):</b>",
        "  Session flags: london_sweep / ny_sweep / internal_sweep / strong_trend",
        "  NY gate: no London sweep → needs NY sweep / strong trend / internal liquidity",
        "  Model 4: exhaustion detector | psych TP | hold behaviour | shutdown rule",
        "  Trade limiter: per-window caps + quality gate + pair cooldown",
        "  Volatility kill filter | Chop filter | BOS model locking",
        "",
        "<b>Bias:</b>  ±4 Strong  |  ±2/3 Lean  |  ±1 Neutral→NO TRADE",
        "<b>Pinned:</b>  BTC ETH SOL BNB XRP ADA AVAX TON HYPE ZEC DASH",
