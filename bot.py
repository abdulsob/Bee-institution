...
def run_state_machine(sym, m15, h4, daily, bd, loc_data, structure, pair_score, market_condition=None, live_price=None):
    ...
    # ── Model 4: Update momentum state on every active cycle (PDF §8) ────────
    # Must run BEFORE entry logic in ALL active states — not just WAITING_FOR_RETRACEMENT.
    # Exhaustion can block entry from any path (Model 1/2/3).
    if st["state"] in ("BEHAVIOR_CLASSIFICATION", "SWEEP_CONFIRMED", "WAITING_FOR_RETRACEMENT"):
        update_model4_state(sym, m15, bias)
        if st.get("exhaustion") and st["state"] != "WAITING_FOR_SWEEP":
            record_rejection(sym, "Model 4 Exhaustion",
                             "Top gainer showing exhaustion — entry blocked (PDF §8)")
            log.info(f"{sym}: MODEL 4 EXHAUSTION — entry blocked early (state={st['state']})")
            return
    ...
    if st["state"] == "WAITING_FOR_RETRACEMENT":
        ...
        # ── Model 4: Dynamic OTE zone using min_retrace (PDF §11) ────────────
        # Standard entry: price must be inside ote_low–ote_high (0.618–0.705 zone).
        # Model 4 priority pair: min_retrace = 0.25 → entry allowed much earlier.
        m4_min_retrace = model4_get_min_retrace(st)
        if m4_min_retrace < 0.5:
            # Earlier entry: compute zone from leg using the Model 4 threshold
            leg_high = ote.get("leg_high", 0)
            leg_low  = ote.get("leg_low",  0)
            leg_rng  = leg_high - leg_low
            if leg_rng > 0:
                if bias == "bullish":
                    m4_ote_low  = leg_high - m4_min_retrace * leg_rng
                    m4_ote_high = ote["ote_high"]   # upper bound stays same
                else:
                    m4_ote_high = leg_low  + m4_min_retrace * leg_rng
                    m4_ote_low  = ote["ote_low"]
                in_zone = m4_ote_low <= price <= m4_ote_high
                log.debug(f"{sym}: Model4 early OTE zone {m4_ote_low:.4f}-{m4_ote_high:.4f} (min_retrace={m4_min_retrace})")
            else:
        ...
        if not in_zone:
            ...
            if mom["valid"]:
                ...
                # ── Model 4: Apply TP multiplier to actual risk levels (PDF §11) ─
                m4_tp_mult   = model4_get_tp_multiplier(st)
                m4_psych_tp  = st.get("tp_target")
                m4_hold_note = ""
                m4_tp_line   = ""
                risk_b_display = dict(risk_b)
                if st.get("momentum_model") and m4_tp_mult != 1.0:
                    entry_val = risk_b_display.get("entry", price)
                    sl_val    = risk_b_display.get("sl",    0)
                    base_rr   = abs(entry_val - sl_val)
                    if base_rr > 0:
                        if bias == "bullish":
                            risk_b_display["tp1"]    = entry_val + base_rr * 2 * m4_tp_mult
                            risk_b_display["tp2"]    = entry_val + base_rr * 3 * m4_tp_mult
                            risk_b_display["runner"] = entry_val + base_rr * 5 * m4_tp_mult
                        else:
                            risk_b_display["tp1"]    = entry_val - base_rr * 2 * m4_tp_mult
                            risk_b_display["tp2"]    = entry_val - base_rr * 3 * m4_tp_mult
                            risk_b_display["runner"] = entry_val - base_rr * 5 * m4_tp_mult
                ...
                send_telegram(
                    ...
                    f"Entry:        <b>{risk_b_display.get('entry', risk_b.get('entry', price)):.4f}</b>\n"
                    f"SL:           <b>{risk_b_display.get('sl', risk_b.get('sl', 0)):.4f}</b>\n"
                    f"TP1 (50%):    <b>{risk_b_display.get('tp1', risk_b.get('tp1', 0)):.4f}</b>  [{risk_b_display.get('tp1_label', risk_b.get('tp1_label', '--'))}]\n"
                    f"TP2 (30%):    <b>{risk_b_display.get('tp2', risk_b.get('tp2', 0)):.4f}</b>  [{risk_b_display.get('tp2_label', risk_b.get('tp2_label', '--'))}]\n"
                    f"Runner (20%): <b>{risk_b_display.get('runner', risk_b.get('runner', 0)):.4f}</b>  [{risk_b_display.get('runner_label', risk_b.get('runner_label', '--'))}]"
                    f"{m4_tp_line}\n\n"
        ...
        if reacted:
            ...
            m4_hold_note = ""
            m4_tp_line   = ""
            risk_display = dict(risk)   # copy so we don't mutate stored state
            if st.get("momentum_model") and m4_tp_mult != 1.0:
                entry_val = risk_display.get("entry", price)
                sl_val    = risk_display.get("sl",    0)
                base_rr   = abs(entry_val - sl_val)
                if base_rr > 0:
                    if bias == "bullish":
                        risk_display["tp1"]    = entry_val + base_rr * 2  * m4_tp_mult
                        risk_display["tp2"]    = entry_val + base_rr * 3  * m4_tp_mult
                        risk_display["runner"] = entry_val + base_rr * 5  * m4_tp_mult
                    else:
                        risk_display["tp1"]    = entry_val - base_rr * 2  * m4_tp_mult
                        risk_display["tp2"]    = entry_val - base_rr * 3  * m4_tp_mult
                        risk_display["runner"] = entry_val - base_rr * 5  * m4_tp_mult
            ...
            send_telegram(
                ...
                f"Entry:        <b>{risk_display.get('entry', risk.get('entry', 0)):.4f}</b>\n"
                f"SL:           <b>{risk_display.get('sl', risk.get('sl', 0)):.4f}</b>\n"
                f"TP1 (50%):    <b>{risk_display.get('tp1', risk.get('tp1', 0)):.4f}</b>  [{risk_display.get('tp1_label', risk.get('tp1_label', '--'))}]\n"
                f"TP2 (30%):    <b>{risk_display.get('tp2', risk.get('tp2', 0)):.4f}</b>  [{risk_display.get('tp2_label', risk.get('tp2_label', '--'))}]\n"
                f"Runner (20%): <b>{risk_display.get('runner', risk.get('runner', 0)):.4f}</b>  [{risk_display.get('runner_label', risk.get('runner_label', '--'))}]"
                f"{m4_tp_line}\n\n"
                f"Rating: <b>{st['setup_score']['rating']}</b>\n"
                f"<b>Risk 1-2% only. Narrative confirmed — execute.</b>"
                f"{m4_hold_note}"
...
if __name__ == "__main__":
    ...
    startup_lines = [
        "<b>BEE-M Alert Bot v18 — Model 4 Momentum Layer (Fully Integrated)</b>",
        "",
        "<b>Master flow:</b>  Narrative → Session flags → Model (locked) → BOS → [M4 check] → Entry",
        "<b>Model 4 rule:</b>  Enhances execution ONLY — never creates trades, never overrides structure",
        "",
        "<b>Model 4 — Momentum Modifier (PDF §3–§14):</b>",
        "  Phase engine: EARLY(8-12) / MID(12-15) / LATE(15-17) / LOW — activates on MID/LATE only",
        "  Priority pairs: Top 20 gainers + losers from MEXC — threshold 0.6, min retrace 0.25",
        "  Standard pairs: threshold 0.7, min retrace 0.5",
        "  TP expansion: ×1.8 top gainer / ×1.5 top loser — applied to ACTUAL risk levels",
        "  Psychological TPs: next level above price from [1,2,3,5,8,10,20,50,100]",
        "  Exhaustion block: runs BEFORE entry on ALL states (not just OTE wait)",
        "    → top gainer + retrace>0.6 OR impulse<0.5 OR 3+ large candles = blocked",
        "  Shutdown rule: retrace>0.5 → momentum_model = False (prevents trend chasing)",
        "  Hold behaviour: early exit disabled when momentum active",
        "  OTE zone: widened to min_retrace for priority pairs (earlier entry allowed)",
        "",
        "<b>Session Context (PDF1):</b>",
        "  London/NY/internal sweep flags per pair — reset daily",
        "  NY filter: no London sweep → need NY sweep OR strong trend OR internal sweep",
        "  No London context → B-grade blocked, A+ only",
        "",
        "<b>Edge Filters (PDF2):</b>",
        "  MIN_BOS_STRENGTH hard gate | Volatility kill filter | Strict model lock",
        "  Trade outcome tracking: entry, sl, tp1, sl_hit, tp_hit, partials",
        "",
