...
def _find_bos_impulse_leg(m15, bias, sweep_idx, bos_level):
    ...
    The correct leg must satisfy ALL three:
      1. It comes AFTER the sweep (starts at or after sweep_idx)
      2. It contains the candle that actually broke the BOS level
      3. It is the cleanest displacement leg — highest body ratio
         and most directional of the candidates

    Returns (leg_low, leg_high, leg_start_idx) or None if not found.

    Bullish: leg goes Low → High, the high breaks the BOS swing level
    Bearish: leg goes High → Low, the low breaks the BOS swing level
    """
    # Search window: from sweep to 20 candles beyond (enough to capture BOS)
    search = m15[sweep_idx: sweep_idx + 20]
    if len(search) < 2:
        return None

    avg_body = (
        sum(abs(c["close"] - c["open"]) for c in m15[max(0, sweep_idx - 10): sweep_idx])
        / max(len(m15[max(0, sweep_idx - 10): sweep_idx]), 1)
    )

    best_leg    = None
    best_ratio  = 0.0

    # Walk through candidate starting points for the impulse leg
    for start in range(len(search) - 1):
        c_start = search[start]

        # Build the leg from this starting candle forward
        # Stop as soon as the BOS level is broken
        if bias == "bullish":
            leg_low   = c_start["low"]
            ...
            for end in range(start, len(search)):
                c = search[end]
                if c["low"] < leg_low:
                    # Leg made a lower low — not a clean upward impulse from here
                    break
                leg_high = max(leg_high, c["high"])
                body = abs(c["close"] - c["open"])
                rng  = c["high"] - c["low"]
                if rng > 0 and c["close"] > c["open"]:   # directional bullish candle
                    total_body += body / rng
                    candle_count += 1
                # Did this candle's close break the BOS level?
                if c["close"] > bos_level and bos_broken_at == -1:
                    bos_broken_at = end
                    break   # leg ends at the BOS break — stop here

            if bos_broken_at == -1:
                continue  # this starting point never reached BOS
            ...
            # Prefer the leg with the highest average body ratio (most displacement)
            if avg_ratio > best_ratio and avg_ratio >= 0.40:
                best_ratio = avg_ratio
                best_leg   = (leg_low, leg_high, sweep_idx + start)
    ...
    return best_leg   # (leg_low, leg_high, abs_start_idx) or None
...
def calc_ote(m15, bias, sweep_idx, bos_level, move_quality="moderate"):
    ...
    Per PDF correction:
    - Fib must be drawn on the leg that actually broke structure.
    - If multiple swings exist, use the cleanest displacement leg.
    - Fallback: use candles from sweep to BOS level if leg detection fails.

    Bullish: Low → High (BOS level is the high)
    Bearish: High → Low (BOS level is the low)
    OTE zone: 0.62 – 0.79 | Ideal: ~0.705
    """
    zones = {
    ...
    if impulse:
        leg_low, leg_high, leg_start = impulse
        log.debug(
            f"OTE: BOS impulse leg found — "
            f"low={leg_low:.4f}  high={leg_high:.4f}  "
            f"start_idx={leg_start}  quality={move_quality}"
        )
    else:
        # Fallback: use raw window from sweep index (old behaviour)
        log.debug("OTE: impulse leg not found — falling back to sweep window")
        leg_window = m15[sweep_idx: sweep_idx + 15]
        if not leg_window:
            return {}
        if bias == "bullish":
            leg_low  = min(c["low"]  for c in leg_window)
            leg_high = bos_level
        else:
            leg_high = max(c["high"] for c in leg_window)
            leg_low  = bos_level
    ...
    if bias == "bullish":
        ...
        return {
            ...
            "leg_source":   "impulse" if impulse else "fallback",
        }
    elif bias == "bearish":
        rng = leg_high - leg_low
        if rng <= 0:
    ...
    for upd in updates:
        ...
        if text in ("/status", "/s"):
            ...
            send_telegram(
                ...
                "/setups  or  /st — <b>Confirmed setups only</b>: Entry, SL, TP for pairs ready to trade\n"
                "/pairs   or  /p  — Active setups: all pairs in a non-waiting state\n"
...
def build_setup_alert(sym, bd, loc_data, sweep, disp, bos,
    ...
    lines += [
        ...
        "",
        "<b>— OTE ENTRY ZONE —</b>",
        f"Leg: {ote.get('leg_low', 0):.4f}–{ote.get('leg_high', 0):.4f}  [source: {ote.get('leg_source','?')}]",
        ...
        if not cont["valid"]:
            ...
            if key not in alerted_today:
                ...
                model_label   = "Model 2 — In-Transit (riding move to POI)" if is_in_transit \
                                else "Model 2 — Trend Continuation"
...
def scan_pair(sym, pair_score):
    ...
    if st["state"] == "WAITING_FOR_SWEEP":
        ...
        # Fix 5: Structure filter is advisory only when sweep+BOS exist
        # Hard block only on truly ranging/choppy markets with no setup forming
        if not structure["clean"]:
            record_rejection(sym, "Step 3 (Structure)", structure["reason"])
            log.info(f"{sym}: {structure['reason']} — will still check for sweep+BOS")
            # Don't return — continue to model selection.
            # If sweep+BOS found, structure filter is overridden.
    else:
...
if __name__ == "__main__":
    ...
    startup_lines = [
        ...
        "",
        "<b>Fix 5 — Structure filter loosened:</b>",
        "  Choppy/ranging flag is advisory — if sweep+BOS exist, trade proceeds",
        "",
        ...
        "<b>Scanning:</b>  24/7 every 15 min | /setups  /pairs  /status  /help",
    ]
    send_telegram("\n".join(startup_lines))
