"""
BEE-M Trading Alert Bot v15  (Structured Flip Model)
...
# ── Trade limiter — PnL fix #1 ──────────────────────────────────────────────
# Your system mandates 3-4 trades per session max. Without this the bot is
# an overtrading machine. Keyed by session name, reset at each new day/session.
MAX_TRADES_PER_SESSION = int(os.environ.get("MAX_TRADES_PER_SESSION", "4"))
session_trades    = {}   # {session_name: count}
session_trade_day = {}   # {session_name: "YYYY-MM-DD"} for daily reset
...
def _check_trade_limit(session_name):
    """
    Returns (allowed: bool, reason: str).
    Resets count at the start of each new UTC day per session.
    """
    if not session_name:
        return True, ""   # outside window — no limit applies
    today = datetime.now(timezone.utc).strftime("%Y-%m-%d")
    if session_trade_day.get(session_name) != today:
        session_trades[session_name]    = 0
        session_trade_day[session_name] = today
    count = session_trades.get(session_name, 0)
    if count >= MAX_TRADES_PER_SESSION:
        return False, (
            f"Trade limit reached for {session_name} "
            f"({count}/{MAX_TRADES_PER_SESSION} today)"
        )
    return True, ""


def _register_trade(session_name, sym, bias, model, bos_strength):
    """
    Increment trade count and append to trade_log.
    Call this exactly once when an alert is actually sent.
    """
    if session_name:
        session_trades[session_name] = session_trades.get(session_name, 0) + 1
    trade_log.append({
        "pair":         sym,
        "model":        model,
        "session":      session_name or "outside_window",
        "bias":         bias,
        "bos_strength": round(bos_strength, 3),
        "time":         datetime.now(timezone.utc).isoformat(),
        "result":       None,   # filled in manually or via future logging hook
    })
    log.info(
        f"TRADE LOGGED — {sym} {bias.upper()} [{model}] "
        f"session={session_name}  "
        f"count={session_trades.get(session_name,0)}/{MAX_TRADES_PER_SESSION}  "
        f"total_logged={len(trade_log)}"
    )
...
def build_trade_log_report():
    ...
    today = datetime.now(timezone.utc).strftime("%Y-%m-%d")
    lines.append("<b>Today's session counts:</b>")
    for sh, sm, eh, em, name in EXECUTION_WINDOWS_WAT:
        day_check = session_trade_day.get(name)
        count     = session_trades.get(name, 0) if day_check == today else 0
        cap_emoji = "🔴" if count >= MAX_TRADES_PER_SESSION else "🟢"
        lines.append(f"  {cap_emoji} {name}: {count}/{MAX_TRADES_PER_SESSION}")
...
def run_state_machine(sym, m15, h4, daily, bd, loc_data, structure, pair_score, market_condition=None, live_price=None):
    ...
    if st["state"] in ("SWEEP_CONFIRMED", "WAITING_FOR_RETRACEMENT"):
        update_narrative(sym, m15, h4, daily, live_price=live_price)

    # ════════════════════════════════════════════════════════════════════════
    # MODEL SELECTION — "Where am I?"
    # ════════════════════════════════════════════════════════════════════════
    use_continuation = (
        mc["condition"] == "trending" and
        mc.get("trend_dir") == bias and
        st["state"] == "WAITING_FOR_SWEEP"   # only pick model on fresh setups
    )

    # ════════════════════════════════════════════════════════════════════════
    # MODEL 2 — CONTINUATION  (trending, not at extremes)
    # Dedicated detection path — does NOT require PDH/PDL sweep
    # ════════════════════════════════════════════════════════════════════════
    if use_continuation:
        ...
        if not cont["valid"]:
            ...
            log.info(f"{sym}: [MODEL B LOCKED] {cont.get('reason', '')} — trending market, Model A BLOCKED")
            # ── MODEL ENFORCEMENT (PDF fix #4) ────────────────────────────────
            # Market is trending → this is Model B territory.
            # If Model B conditions aren't met yet, we WAIT — we do NOT fall
            # through to Model A (Reversal). That's a different market context.
            # "Reversal → Model A | Continuation → Model B — no mixing."
            return
        else:
            ...
            if key not in alerted_today:
                in_window_cont, win_name_cont = in_execution_window()

                # ── TRADE LIMITER — Model B (PnL fix #1) ─────────────────────
                if in_window_cont:
                    session_name_b = _current_session_name()
                    allowed_b, limit_reason_b = _check_trade_limit(session_name_b)
                    if not allowed_b:
                        record_rejection(sym, "Trade Limiter (Model B)", limit_reason_b)
                        log.info(f"{sym}: TRADE LIMIT BLOCKED (Model B) — {limit_reason_b}")
                        send_telegram(
                            f"<i>⛔ Trade limit reached — {sym.replace('_', '/')} {bias.upper()} continuation blocked\n"
                            f"{limit_reason_b}\n"
                            f"Setup is valid but cap is enforced. Wait for next session.</i>"
                        )
                ...
                model_label   = "Model 2 — In-Transit (riding move to POI)" if is_in_transit \
                                else "Model 2 — Trend Continuation (Model B)"
                log.info(f"{sym}: CONTINUATION SETUP [{model_label}] — {cont['entry_type']}")
                send_telegram(
                    ...
                    f"TP2 (30%):    <b>{tp2:.4f}</b>  ({'POI target' if is_in_transit else 'impulse high/low'})\n"
                    f"Runner (20%): <b>{runner:.4f}</b>  (external liquidity)\n\n"
                    f"Bias score: {bd['score']:+d}  |  Location: {loc.upper()}\n"
                    f"<b>Risk 1-2% only.</b>"
                )
                # Register in trade log
                session_name_b2 = _current_session_name()
                _register_trade(session_name_b2, sym, bias, model_label, 0.0)
    ...
    if st["state"] == "WAITING_FOR_SWEEP":
        ...
        # Mark that this pair had a London sweep today so the NY session gate passes
        in_window_sw, win_name_sw = in_execution_window()
        if in_window_sw and "Primary" in win_name_sw:
    ...
    if st["state"] == "SWEEP_CONFIRMED":
        ...
        # ── BOS STRENGTH FILTER (PnL fix #2) ────────────────────────────────
        # Only strong displacement BOS matters. Weak/chop breaks = fake structure.
        # Threshold: body ratio >= 0.70 (strong displacement candle).
        bos_body_ratio = bos.get("body_ratio", 0)
        if bos_body_ratio < 0.70:
            reason = (
                f"BOS rejected — weak displacement body ({bos_body_ratio:.0%} < 70%). "
        ...
        if key not in alerted_today:
            ...
            # ── TRADE LIMITER (PnL fix #1) ───────────────────────────────────
            # Block if session trade cap already hit. Only gate INSIDE windows —
            # outside window setups are held/informational so don't count against cap.
            if in_window:
                session_name = _current_session_name()
                allowed, limit_reason = _check_trade_limit(session_name)
                if not allowed:
                    record_rejection(sym, "Trade Limiter", limit_reason)
                    log.info(f"{sym}: TRADE LIMIT BLOCKED — {limit_reason}")
                    send_telegram(
                        f"<i>⛔ Trade limit reached — {sym.replace('_', '/')} {bias.upper()} setup blocked\n"
                        f"{limit_reason}\n"
                        f"Setup is valid but cap is enforced. Wait for next session.</i>"
                    )
                    return
            ...
            # Register trade in log and increment session counter
            session_name = _current_session_name()
            _register_trade(session_name, sym, bias, enforced_model, bos.get("body_ratio", 0))
...
def scan_pair(sym, pair_score):
    ...
    # ── SESSION BEHAVIOUR GATE (PnL fix #5) ─────────────────────────────────
    # Your system: London creates the move, NY sweeps/continues it.
    # Rule: if we are in the Second Wave (NY) session and this pair has NOT
    # had a sweep confirmed in the Primary (London) session today, require
    # that a liquidity sweep already occurred before looking for NY entries.
    # Active setups (already in SWEEP_CONFIRMED etc.) are always allowed through.
    in_window_now, win_name_now = in_execution_window()
    st_for_gate = pair_states.get(sym, {})
    is_active_setup = st_for_gate.get("state") not in (None, "WAITING_FOR_SWEEP")
    if (in_window_now
            and "Second Wave" in win_name_now
            and not is_active_setup):
        # Check if a London sweep happened today for this pair
        london_key = f"{sym}_london_swept_{datetime.now(timezone.utc).strftime('%Y-%m-%d')}"
        if london_key not in alerted_today:
            record_rejection(
                sym, "Session Gate",
                "Second Wave (NY) — no London sweep on record today. "
                "Require prior liquidity sweep to trade NY session."
            )
            log.info(f"{sym}: SESSION GATE — skipping NY scan, no London sweep today")
            return
        log.info(f"{sym}: SESSION GATE — London sweep confirmed, NY scan allowed")
...
if __name__ == "__main__":
    log.info("BEE-M Alert Bot v15 starting...")

    startup_lines = [
        "<b>BEE-M Alert Bot v15 — In-Transit Model</b>",
        "",
        "<b>Master flow:</b>  Narrative → Where am I? → Model → BOS → Entry → (Flip?)",
        "",
        "<b>Fix 1 — Real Continuation Model:</b>",
        "  Impulse → Pullback 20-75% → HL/LH holds → Internal sweep/compression → BOS",
        "  ✅ No PDH/PDL sweep needed — pure trend structure detection",
        "",
        "<b>Fix 2 — Adaptive Narrative:</b>",
        "  Narrative rebuilds every cycle — alerts you if bias or condition changes",
        "  📋 NARRATIVE UPDATE message fires when story shifts",
        "",
        "<b>Fix 3 — Two Flip Modes:</b>",
        "  🛡 Conservative: Opposite sweep + Displacement + BOS",
        "  ⚡ Aggressive: Strong displacement (≥70% body) + BOS — no sweep needed",
        "  Aggressive fires first to catch faster reversals",
        "",
        "<b>Fix 4 — Explicit Where Am I? Layer:</b>",
        "  extreme → Model 1 Reversal  |  trending → Model 2 Continuation",
        "  ranging → Reversal (wait for sweep)  |  Checked before any model runs",
        "",
        "<b>Fix 5 — PnL Fixes Applied (v16):</b>",
        "  1. Trade Limiter: max 4 trades/session (env: MAX_TRADES_PER_SESSION)",
        "  2. BOS Strength: body ratio ≥ 70% required — chop breaks rejected",
        "  3. Chop Filter: HARD block — ranging market = no trade",
        "  4. Model Enforcement: Reversal=Model A  |  Continuation=Model B (strict)",
        "  5. Session Logic: NY session requires London sweep evidence",
        "  6. Narrative Gate: bias score |±1| = NO TRADE (hard enforced)",
        "  7. Trade Log: /tradelog or /tl to view all fired setups",
        "",
        "<b>Opposite Sweep Rules (PDF):</b>",
        "  Invalidates ONLY if: major level (PDH/PDL/EQL) + displacement + before BOS",
        "  Minor sweeps / no displacement = ignored (liquidity engineering)",
        "",
        "<b>Bias:</b>  ±4 Strong  |  ±2/3 Lean  |  ±1 Neutral→NO TRADE",
        "<b>Location:</b>  Bullish in discount ✔  |  Bearish in premium ✔",
        "<b>Pinned pairs:</b>  BTC ETH SOL BNB XRP ADA AVAX TON HYPE ZEC DASH (always scanned)",
        "<b>Expiry:</b>  Sweep + BOS must complete within the same kill zone window (+ 15min grace)",
