    ...
    if use_continuation:
        ...
        if not cont["valid"]:
            ...
            if key not in alerted_today:
                ...
                # ── TRADE LIMITER — Model B — 3 layers ───────────────────────
                if in_window_cont:
                    # Model B setups are ungraded (no score_setup call) — treat as "A"
                    # since Model B only fires when trending is confirmed (already filtered).
                    allowed_b, limit_reason_b = _check_trade_limit(win_name_cont, sym, "A")
                    if not allowed_b:
                        record_rejection(sym, "Trade Limiter (Model B)", limit_reason_b)
                        log.info(f"{sym}: TRADE LIMIT BLOCKED (Model B) — {limit_reason_b}")
                        send_telegram(
                            f"<i>⛔ Blocked — {sym.replace('_', '/')} {bias.upper()} continuation\n"
                            f"{limit_reason_b}</i>"
                        )
                ...
                _register_trade(win_name_cont, sym, bias, model_label, 0.0, "A")
...
if __name__ == "__main__":
    ...
    startup_lines = [
        ...
        "  1. Trade Limiter: per-window caps (Primary=2, Optional=1, SecondWave=2)",
        "     + Quality gate: B-grade blocked at 50%+ window capacity",
        "     + Same-pair cooldown: pair locked for rest of window after alert",
