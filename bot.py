"""
BEE-M Trading Alert Bot v13  (Structured Flip Model)
...
def build_narrative(bd, loc_data, m15, market_condition=None):
    """
...
def update_narrative(sym, m15, h4, daily):
    """
    Called every scan cycle for pairs in active states.
    Rebuilds the narrative if market context has changed since the
    original narrative was built (e.g. bias shift, structure breakdown).
    Sends a Telegram alert if the narrative materially changes.
    """
    ...
    # Recompute bias and condition from current data
    bd      = daily_bias(daily)
    loc     = location_4h(h4)
    mc      = classify_market_condition(h4, daily, bd)
    new_narr = build_narrative(bd, loc, m15, market_condition=mc)
...
def classify_market_condition(h4, daily, bd):
    ...
    # ── 1. Am I at extremes? ─────────────────────────────────────────────────
    # Extreme = within 8% of PDH or PDL, OR near a clear 4H swing extreme
    recent_4h = h4[-20:]
    h4_high   = max(c["high"] for c in recent_4h)
    h4_low    = min(c["low"]  for c in recent_4h)
    h4_rng    = h4_high - h4_low if h4_high != h4_low else 1

    near_pdh  = pd_range > 0 and (price >= pdh - pd_range * 0.08)
    near_pdl  = pd_range > 0 and (price <= pdl + pd_range * 0.08)
    near_h4_high = (price >= h4_high - h4_rng * 0.08)
    near_h4_low  = (price <= h4_low  + h4_rng * 0.08)
...
def run_state_machine(sym, m15, h4, daily, bd, loc_data, structure, pair_score, market_condition=None):
    st        = get_state(sym)
    bias      = bd["bias"]
    ...
    if st["state"] in ("SWEEP_CONFIRMED", "WAITING_FOR_RETRACEMENT"):
        update_narrative(sym, m15, h4, daily)
    ...
    if use_continuation:
        ...
        if not cont["valid"]:
            ...
            narr    = build_narrative(bd, loc_data, m15, market_condition=mc)
...
if __name__ == "__main__":
    log.info("BEE-M Alert Bot v13 starting...")

    startup_lines = [
        "<b>BEE-M Alert Bot v13 — Five-Fix Edition</b>",
        "",
